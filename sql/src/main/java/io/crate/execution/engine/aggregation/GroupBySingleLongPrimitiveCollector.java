...
public final class GroupBySingleLongPrimitiveCollector implements Collector<Row, GroupBySingleLongPrimitiveCollector.Groups, Iterable<Row>> {
    ...
    GroupBySingleLongPrimitiveCollector(CollectExpression<Row, ?>[] expressions,
                                        AggregateMode mode,
    ...
    private void reduce(Groups groups, Row row) {
        ...
        if (keyInput.hasValue()) {
            long key = keyInput.getLong();
